...
from tensorless.backends.mlx_backend import MlxTinyTransformer


@pytest.mark.parametrize("task", ["text-generation", "text-classification"])
def test_mlx_transformer_forward_backward_and_state_round_trip(task):
    classes = 3 if task == "text-classification" else 0
    model = MlxTinyTransformer(
        vocab_size=12,
        d_model=4,
        layers=1,
        heads=2,
    ...
    restored = MlxTinyTransformer(12, 4, 1, 2, 2, 0.0, 6, task=task, n_classes=classes)
    restored.load_state_dict(state)
    np.testing.assert_allclose(
        restored.forward(input_ids, None if task == "text-generation" else mask), logits
    )
